...
def get_parser() -> tuple:
    parser = argparse.ArgumentParser(
        description="Get some values for our lab :")
    parser.add_argument("text", type=str, help="Text for some next operations")
    parser.add_argument("k", nargs="?", default=10, type=int,
                        help="Need some value of k, default = 10")
    parser.add_argument("n", nargs="?", default=4, type=int,
                        help="Need some value of n, default = 4")
...
def average_amount_words(sentanses: list) -> int:
    return(mean([len(sentanse) for sentanse in sentanses]))


def median_amount_words(sentanses: list) -> int:
    return(median([len(sentanse) for sentanse in sentanses]))
...
def sorted_grams(grams_dict: dict) -> dict:
    sorted_grams_dict = dict(
        sorted(grams_dict.items(), key=lambda kv: kv[1], reverse=True))
